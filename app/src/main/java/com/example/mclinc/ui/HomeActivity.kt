...
import com.example.mclinc.databinding.ActivityHomeBinding
...
class HomeActivity : AppCompatActivity() {
    ...
    override fun onBackPressed() {
        ...
        // Check if we are at the start destination
        if (navController.currentDestination?.id == navController.graph.startDestinationId) {
            if (timer + 2000L > System.currentTimeMillis()) {
                ...
                Toast.makeText(
                    applicationContext,
                    getString(R.string.press_once_again_to_exit),
                    Toast.LENGTH_SHORT
                ).show()
